...
void draw()
{
  ...
  stroke(0);
  float mult = 20;
  if(maxspectrum > 40.0 && maxfrequency < 60) {
  ...
  int output = int(filtered[0]*200*v);
 ...
 // if(frameCount%5 == 0) myPort0.write(30-output);
  ...
  for (Map.Entry entry : fingerPositions.entrySet())
  {
    ...
    if(fingers==1 && fingerId%10==1) {
      ...
      int ac = int(position.getZ());
      if(ac>50) ac = 50; 
      if(ac<-50) ac = -50;
      write_ac = int(map(ac, -50, 50, 0, 40));
      // myPort0.write(write_ac);
      
      int zc = int(position.getY());
      inheight = zc;
      ...
      if(zc<40) zc = 40;
      if(zc>120) zc = 120; 
      write_zc = int(map(zc, 40, 120, 0, 60));
  ...
  // v1 = v1;
  // println("v1", v1);
  if(inpitch != 0) v1 += int(inpitch*p);
  if(inheight !=0) v1 += int(inheight*0.2*h);
  println(v1);
